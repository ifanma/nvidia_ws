...
int main(int argc, char *argv[])
{
    ...
    ros::Subscriber sub = nh.subscribe(topic_, 10, arrayCallback);

    ros::Rate loop_rate(200);

    //Read launch file params
    nh.getParam("hand_id", hand_id_);
    nh.getParam("portname", port_name_);
    nh.getParam("baudrate", baudrate_);
    nh.getParam("test_flags", test_flags);

    std::cout<< hand_id_<< port_name_<< std::endl;
